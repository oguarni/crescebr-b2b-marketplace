...
const HomePage: React.FC = () => {
  ...
  if (isAuthenticated && user?.role === 'admin') {
    return (
      <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', pb: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', py: 8 }}>
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', pb: 10, display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <Box component="header" sx={{ position: 'sticky', top: 0, zIndex: 40, bgcolor: 'background.paper', borderBottom: 1, borderColor: 'divider', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
        <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
          <Box sx={{ flex: 1, position: 'relative' }}>
            <Search sx={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'text.secondary', fontSize: 20, zIndex: 1 }} />
            <InputBase
              fullWidth
          ...
          >
            <Badge color={showAdvancedFilters ? "primary" : "default"} variant="dot" sx={{ '& .MuiBadge-badge': { right: 2, top: 2 } }}>
          ...
          <IconButton
            ...
            sx={{ bgcolor: 'grey.100', borderRadius: 2, p: 1, '&:hover': { bgcolor: 'grey.200' } }}
          >
            <Badge badgeContent={cartItems.length} color="error" sx={{ '& .MuiBadge-badge': { fontSize: '0.625rem', height: 16, minWidth: 16 } }}>
        ...
        {/* Categories Scroller */}
        <Box sx={{ px: 2, pb: 1.5, display: 'flex', gap: 1, overflowX: 'auto', '&::-webkit-scrollbar': { display: 'none' }, msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
          {categories.length > 0 ? categories.map((category) => (
            ...
            ['All Products', 'Machinery', 'Safety Gear', 'Hydraulics', 'Electrical', 'Raw Materials'].map((cat) => (
              <Button
                key={cat}
      ...
      <Collapse in={showAdvancedFilters}>
        <Paper elevation={0} sx={{ m: 2, p: 2, border: 1, borderColor: 'divider', borderRadius: 2, bgcolor: 'grey.50' }}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 500, mb: 1 }}>
            ...
            {/* Availability Status */}
            <Grid item xs={12} md={6}>
              <Typography variant="caption" sx={{ fontWeight: 500, mb: 1, display: 'block' }}>Availability Status</Typography>
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                {['in_stock', 'limited', 'custom_order'].map(status => (
                  <FormControlLabel
                    ...
                    label={<Typography variant="caption">{status === 'in_stock' ? 'In Stock' : status === 'limited' ? 'Limited' : 'Made to Order'}</Typography>}
                  />
                ))}
              </Box>
            </Grid>
          ...
          <Grid container spacing={2}>
            {products.map((product) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={product.id}>
                ...
                >
                  <Box sx={{ position: 'relative', width: '100%', paddingTop: '75%', bgcolor: 'grey.100', overflow: 'hidden' }}>
                    <CardMedia
                      component="img"
                    ...
                    />
                    <Box sx={{ position: 'absolute', top: 8, right: 8, bgcolor: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(4px)', px: 1, py: 0.5, borderRadius: 1, fontSize: '0.625rem', fontFamily: 'monospace', fontWeight: 700, color: 'text.secondary', border: 1, borderColor: 'divider' }}>
                      SKU-{product.id || Math.floor(Math.random() * 9000) + 1000}
                    </Box>
                    {product.stockQuantity > 0 && (
                      <Box sx={{ position: 'absolute', top: 0, left: 0, bgcolor: 'success.main', color: 'success.contrastText', px: 1, py: 0.5, borderBottomRightRadius: 4, fontSize: '0.625rem', fontWeight: 700 }}>
                        IN STOCK
                      </Box>
                    )}
                  </Box>
                  ...
                  <CardContent sx={{ p: 2, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    <Box sx={{ mb: 0.5 }}>
                      <Typography variant="caption" sx={{ fontWeight: 700, color: 'primary.main', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.625rem' }}>
                        {product.category || 'Uncategorized'}
                      </Typography>
                    </Box>

                    <Typography variant="subtitle2" sx={{ fontWeight: 600, color: 'text.primary', lineHeight: 1.2, mb: 1, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: '2.4em' }}>
                      {product.name}
                    </Typography>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 2 }}>
                      ...
                      <Typography variant="caption" color="text.secondary" noWrap>
                        {product.supplierId ? `Supplier ID: ${product.supplierId}` : 'Industrias Brasil Ltda.'}
                      </Typography>
                    </Box>

                    <Box sx={{ mt: 'auto', pt: 1.5, borderTop: 1, borderColor: 'divider', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <Typography variant="caption" sx={{ fontSize: '0.625rem', color: 'text.secondary' }}>Unit Price</Typography>
                        <Typography variant="subtitle1" sx={{ fontFamily: 'monospace', fontWeight: 700, color: 'text.primary', lineHeight: 1 }}>
      ...
      {/* Bottom Navigation */}
      <Box sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, bgcolor: 'background.paper', borderTop: 1, borderColor: 'divider', zIndex: 50, boxShadow: '0 -4px 6px -1px rgba(0,0,0,0.05)' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 3, py: 1 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, color: 'primary.main', cursor: 'pointer' }}>
            <GridViewOutlined fontSize="small" />
            <Typography variant="caption" sx={{ fontSize: '0.625rem', fontWeight: 500 }}>Catalog</Typography>
          </Box>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, color: 'text.secondary', cursor: 'pointer', '&:hover': { color: 'primary.main' } }} onClick={() => navigate('/my-quotations')}>
            <RequestQuoteOutlined fontSize="small" />
            <Typography variant="caption" sx={{ fontSize: '0.625rem', fontWeight: 500 }}>Quotes</Typography>
          </Box>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, color: 'text.secondary', cursor: 'pointer', '&:hover': { color: 'primary.main' } }} onClick={() => navigate('/my-orders')}>
            <LocalShippingOutlined fontSize="small" />
            <Typography variant="caption" sx={{ fontSize: '0.625rem', fontWeight: 500 }}>Orders</Typography>
          </Box>
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, color: 'text.secondary', cursor: 'pointer', '&:hover': { color: 'primary.main' } }} onClick={() => navigate('/login')}>
            <PersonOutlined fontSize="small" />
            <Typography variant="caption" sx={{ fontSize: '0.625rem', fontWeight: 500 }}>Account</Typography>
          </Box>
        </Box>
      </Box>
    </Box>
